...
window_copy_next_paragraph(struct window_mode_entry *wme)
{
	...
	ox = window_copy_find_length(wme, oy);
	window_copy_scroll_to(wme, ox, oy, 1);
}
	...
	retry:
		...
		if (gc.data.size != 1 || (gc.flags & GRID_FLAG_PADDING))
			...
			if (cp != NULL && data->modekeys == MODEKEY_VI) {
				...
				window_copy_scroll_to(wme, px, py, 1);
...
static void
window_copy_scroll_to(struct window_mode_entry *wme, u_int px, u_int py, int direction)
{
	struct window_copy_mode_data	*data = wme->data;
    ...
    int direction, int regex)
{
	...
	if (found) {
		window_copy_scroll_to(wme, px, i, direction);
		return (1);
	}
	if (wrap) {
